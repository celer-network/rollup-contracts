...
contract TransitionEvaluator {
    ...
    function evaluateTransition(
        ...
        dt.StorageSlot[] memory storageSlots = _storageSlots;
        ...
        returns (bytes32)
    {
        return
            keccak256(
                abi.encode(
                    _withdrawTx.accountAddress,
                    _withdrawTx.tokenAddress,
                    _withdrawTx.amount
                )
            );
    }
    ...
    function applyInitialDepositTransition(
        ...
        _storageSlot.value.accountAddress = _transition.accountAddress;
    ...
    function applyDepositTransition(
        ...
        address accountAddress = _storageSlot.value.accountAddress;
    ...
    function applyTransferTransition(
        ...
        address sender = _storageSlots[0].value.accountAddress;
        address recipient = _storageSlots[1].value.accountAddress;
    ...
    {
        (uint256 transitionType, bytes32 stateRoot, uint256 accountSlotIndex, address accountAddress, uint256 tokenIndex, uint256 amount, bytes memory signature) = abi
            .decode(
            (_rawBytes),
